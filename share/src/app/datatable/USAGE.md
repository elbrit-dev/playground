...
- **`overrides.token`**: When it is a non-empty string (after trim), it is sent as the **`Authorization`** header for all datatable GraphQL traffic for that provider: main query pipeline (worker), **nested queries called from a transformer's `queryFunction`**, index checks, month fetches, write-schema introspection, save (`bulkCreate` / `bulkUpdate`) mutations, and the **background `clientSave` index pull** (the batch freshness sweep run once per provider mount over every saved query with `clientSave === true`). Nested queries still resolve their own **endpoint** from their own `urlKey` (they may live on a different environment), but the override token always wins over whatever token that `urlKey` would otherwise resolve to.
