...
export async function executeGraphQLQuery(queryDoc, options = {}) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Invalid JSON response from GraphQL endpoint: ${responseText.substring(0, 200)}`
      );
    } catch (textError) {
      throw new Error(
        `Failed to parse GraphQL response: ${parseError.message}`
      );
    }
  }
...
export async function executePipeline(queryId, context, options = {}) {
  ...
  const chain = context.dependencyStack.length > 0 
    ? `${context.dependencyStack.join(' → ')} → ${queryId}`
    : queryId;
  ...
  try {
    ...
    const queryFunction = async (nestedQueryId) => {
      ...
      return executePipeline(nestedQueryId, context, { endpointUrl: finalEndpointUrl, authToken: finalAuthToken });
    };
